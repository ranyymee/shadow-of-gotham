/*
 * main.c — Intégration lot back + lot joueur + lot NPC
 *
 * Compilation :
 *   gcc -Wall -std=c99 -o game main.c player.c back.c npc.c -lSDL2 -lSDL2_image -lSDL2_ttf -lm
 */
                    ...
                    int *timeLeft, Uint32 *lastTime)
{
    ...
    initPlayer(&p1, sx1, sy, 0);
    initPlayer(&p2, sx2, sy, 1);
    p2.direction = 0;

    loadPlayerTextures(ren, &p1);
    loadPlayerTextures(ren, &p2);

    NPC_init(&gameNPC, ren, "assets/back.png");
    NPC_loadLevel(&gameNPC, ren, 1);
    gameNPC.pHealth = &p1.hp;
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_KEYDOWN && e.key.keysym.scancode == SDL_SCANCODE_ESCAPE)
                { running = 0; break; }
            gererGuideEtClic(e, &bg->guide, &bg->commentJouer, &bg->afficherCommentJouer);
        }
        ...
        if (p1.x < 0) { p1.x = 0; p1.vitesse = 0; }
        if (p1.x > screenW - p1.w) { p1.x = screenW - p1.w; p1.vitesse = 0; }
        if (p2.x < 0) { p2.x = 0; p2.vitesse = 0; }
        if (p2.x > screenW - p2.w) { p2.x = screenW - p2.w; p2.vitesse = 0; }
        ...
        if (affMode == MODE_MULTI) {
            int vpW = screenW / 2;
            SDL_Rect vp1r = {0, 0, vpW, screenH};
            SDL_Rect vp2r = {vpW, 0, vpW, screenH};

            SDL_RenderSetViewport(ren, &vp1r);
            afficherBackgroundEtElements(ren, bg, platforms, taille, font,
                (SDL_Color){255,255,255,255}, *timeLeft, p1.vies,
                bg->camera_pos.x, bg->camera_pos.y, vpW, screenH, MODE_MULTI);
            ...
            SDL_RenderSetViewport(ren, &vp2r);
            afficherBackgroundEtElements(ren, bg, platforms, taille, font,
                (SDL_Color){255,255,255,255}, *timeLeft, p2.vies,
                bg->camera_pos.x, bg->camera_pos.y, vpW, screenH, MODE_MULTI);
            NPC_draw(&gameNPC, bg->camera_pos.x, bg->camera_pos.y);
            blitPlayer(ren, &p1);
            renderBullets(ren, &p1);
            ...
            SDL_RenderSetViewport(ren, NULL);
            afficherBackgroundEtElements(ren, bg, platforms, taille, font,
                (SDL_Color){255,255,255,255}, *timeLeft, (p1.vies + p2.vies) / 2,
                bg->camera_pos.x, bg->camera_pos.y, screenW, screenH, MODE_MONO);
            NPC_draw(&gameNPC, bg->camera_pos.x, bg->camera_pos.y);
            blitPlayer(ren, &p1);
            renderBullets(ren, &p1);
            blitPlayer(ren, &p2);
    ...
    return gameOverScreen(ren, font, bg->img[0], winner, p1.score, p2.score, screenW, screenH);
}
