                          ...
                          Uint32 elapsed,Uint8 tR,Uint8 tG)
{
    ...
    /* Time digits — left of bar */
    {
        int rm=(state==ST_PLAY)?(int)(TIMER_MS-elapsed):0;
        if(rm<0)rm=0;
        char tb[16]; snprintf(tb,16,"%02d:%02d",rm/60000,(rm%60000)/1000);
        SDL_Rect tt={962,6,70,30};
        FR(tt,0,18,38,215); SR(tt,AM_R,AM_G,AM_B,185,1); CO(tt,AM_R,AM_G,AM_B,135,7);
        if(rm<=10000&&state==ST_PLAY){
            float p=0.5f+0.5f*sinf((float)now/140.0f);
            TC(tb,tt,255,(Uint8)(60+p*60),0,g_fntMid);
        } else {
