...
/* SUSPENSE_DELAY is defined in enigme.h — do not redefine here */

/* ============================================================
   loadTexture  (defined once, at the bottom — forward decl not needed
   because jouerSon / initEnigme call it after this definition)
   ============================================================ */
...
{
    if (!e) return;
    if ((int)e->currentSound == id && Mix_PlayingMusic()) return;
    Mix_HaltMusic();
    e->currentSound = id;
