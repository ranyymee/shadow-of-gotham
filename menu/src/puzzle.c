...
/* Definition of PUZZLE_IMAGES (declared extern in header.h) */
const char *PUZZLE_IMAGES[NUM_PUZZLES] = {
    "assets/puzzle.png",
    "assets/puzzle1.png",
    "assets/puzzle2.png",
    "assets/puzzle3.png"
                      ...
                      Uint32 ticks, int backHov, int backClicked)
{
    ...
    /* Restore centered viewport */
    {int _ow=WIN_W,_oh=WIN_H;
     SDL_GetRendererOutputSize(ren,&_ow,&_oh);
     SDL_Rect _vp={(_ow-WIN_W)/2,(_oh-WIN_H)/2,WIN_W,WIN_H};
     SDL_RenderSetViewport(ren,&_vp);}
    SDL_SetRenderDrawColor(ren,0,0,0,255); SDL_RenderClear(ren);
    ...
    {
        SDL_Rect backR={PUZZ_BACK_X,PUZZ_BACK_Y,PUZZ_BACK_W,PUZZ_BACK_H};
...
int runMenuPuzzle(SDL_Window *win, ContextPuzzle *ctx)
{
    ...
    SDL_Rect backRect={PUZZ_BACK_X,PUZZ_BACK_Y,PUZZ_BACK_W,PUZZ_BACK_H};
    int backHov=0, backClicked=0, selected=-1, running=1;
    int mouseLX=0, mouseLY=0;
    float dt=1.0f/60.0f;
    SDL_Event ev;

    /* Centre le contenu WIN_W x WIN_H dans le renderer fullscreen */
    {int _ow=WIN_W,_oh=WIN_H;
     SDL_GetRendererOutputSize(ctx->ren,&_ow,&_oh);
     SDL_Rect _vp={(_ow-WIN_W)/2,(_oh-WIN_H)/2,WIN_W,WIN_H};
     SDL_RenderSetViewport(ctx->ren,&_vp);}
...
int runPuzzle(SDL_Window *win, ContextPuzzle *ctx, int puzzleIdx)
{
    ...
    SDL_Event ev;
    SDL_Rect gridSlot[GPIECES], backBtn={17,871,150,56};
    SDL_Rect hintBtn={1629,231,265,187};
    ...
    {int _ow=WIN_W,_oh=WIN_H;
     SDL_GetRendererOutputSize(ren,&_ow,&_oh);
     _vpX=(_ow-WIN_W)/2; _vpY=(_oh-WIN_H)/2;
     SDL_Rect _vp={_vpX,_vpY,WIN_W,WIN_H};
     SDL_RenderSetViewport(ren,&_vp);}
    ...
    while(running){
        ...
        while(SDL_PollEvent(&ev)){
            ...
            if(ev.type==SDL_MOUSEBUTTONDOWN&&ev.button.button==SDL_BUTTON_LEFT){
                ...
                if(state!=ST_PLAY){
                    int panH=406,panY=WIN_H/2-panH/2;
        ...
        /* Clear full screen black (outside viewport) then set centered+shake viewport */
        SDL_RenderSetViewport(ren,NULL);
        SDL_SetRenderDrawColor(ren,0,0,0,255); SDL_RenderClear(ren);
        {SDL_Rect vp={_vpX+shake.ox,_vpY+shake.oy,WIN_W,WIN_H};SDL_RenderSetViewport(ren,&vp);}
        SDL_SetRenderDrawColor(ren,0,0,0,255); SDL_RenderClear(ren);
...

        {
            ...
            {SDL_Rect tb={0,0,WIN_W,69};SDL_RenderFillRect(ren,&tb);}
            SDL_SetRenderDrawColor(ren,CY_R,CY_G,CY_B,60);SDL_RenderDrawLine(ren,0,69,WIN_W,69);
            SDL_Rect tbar={105,19,1535,25};
...

        {
            ...
            {SDL_Rect hdr={14,78,398,34};SDL_RenderFillRect(ren,&hdr);}
            {SDL_Color tc={CY_R,CY_G,CY_B,255};SDL_Surface *su=TTF_RenderText_Blended(ctx->fntSm,"REFERENCE",tc);
            if(su){SDL_Texture *t=SDL_CreateTextureFromSurface(ren,su);SDL_Rect d={24,81,su->w,su->h};if(t){SDL_RenderCopy(ren,t,NULL,&d);SDL_DestroyTexture(t);}SDL_FreeSurface(su);}}
            if(ctx->puzzTex){SDL_Rect ref={21,112,384,306};SDL_RenderCopy(ren,ctx->puzzTex,NULL,&ref);}
            {char pg[32];snprintf(pg,32,"%d / %d",correctCount,GPIECES);
            SDL_Rect pl={14,427,398,50};
...

        {
            ...
            SDL_SetRenderDrawColor(ren,0,28,62,230);
            {SDL_Rect hdr={1604,78,300,34};SDL_RenderFillRect(ren,&hdr);}
            {SDL_Color tc={CY_R,CY_G,CY_B,255};SDL_Surface *su=TTF_RenderText_Blended(ctx->fntSm,"SYS LOG",tc);
            if(su){SDL_Texture *t=SDL_CreateTextureFromSurface(ren,su);SDL_Rect d={1616,82,su->w,su->h};if(t){SDL_RenderCopy(ren,t,NULL,&d);SDL_DestroyTexture(t);}SDL_FreeSurface(su);}}
            {SDL_Color tc={GR_R,GR_G,GR_B,255};SDL_Surface *su=TTF_RenderText_Blended(ctx->fntTiny,"INTEGRITY",tc);
            if(su){SDL_Texture *t=SDL_CreateTextureFromSurface(ren,su);SDL_Rect d={1620,121,su->w,su->h};if(t){SDL_RenderCopy(ren,t,NULL,&d);SDL_DestroyTexture(t);}SDL_FreeSurface(su);}}
            {SDL_Rect bg={1620,140,261,15},fg={1620,140,(int)(261*correctCount/GPIECES),17};
            ...
            if(su){SDL_Texture *t=SDL_CreateTextureFromSurface(ren,su);SDL_Rect d={1620,168,su->w,su->h};if(t){SDL_RenderCopy(ren,t,NULL,&d);SDL_DestroyTexture(t);}SDL_FreeSurface(su);}}
            {Uint8 tR=(Uint8)(255*(1-timerFrac)),tG=(Uint8)(210*timerFrac);
            SDL_Rect bg={1620,187,261,15},fg={1620,187,(int)(261*timerFrac),17};
            SDL_SetRenderDrawColor(ren,0,18,38,165);SDL_RenderFillRect(ren,&bg);
            SDL_SetRenderDrawColor(ren,tR,tG,40,205);SDL_RenderFillRect(ren,&fg);
            SDL_SetRenderDrawColor(ren,CY_R,CY_G,CY_B,52);SDL_RenderDrawRect(ren,&bg);}
            SDL_SetRenderDrawColor(ren,CY_R,CY_G,CY_B,35);SDL_RenderDrawLine(ren,1620,215,1892,215);
            ...
            SDL_SetRenderDrawColor(ren,CY_R,CY_G,CY_B,30);SDL_RenderDrawLine(ren,1620,452,1892,452);
            {SDL_Color tc={CY_R,CY_G,CY_B,255};SDL_Surface *su=TTF_RenderText_Blended(ctx->fntTiny,"CIPHER:",tc);
            if(su){SDL_Texture *t=SDL_CreateTextureFromSurface(ren,su);SDL_Rect d={1620,462,su->w,su->h};if(t){SDL_RenderCopy(ren,t,NULL,&d);SDL_DestroyTexture(t);}SDL_FreeSurface(su);}}
            {SDL_Rect hd={1613,481,272,50};SDL_SetRenderDrawColor(ren,0,14,30,205);SDL_RenderFillRect(ren,&hd);SDL_SetRenderDrawColor(ren,CY_R,CY_G,CY_B,58);SDL_RenderDrawRect(ren,&hd);
            SDL_Color tc={CY_R,CY_G,CY_B,255};
            ...
            if(su){SDL_Texture *t=SDL_CreateTextureFromSurface(ren,su);SDL_Rect d={1620,572,su->w,su->h};if(t){SDL_RenderCopy(ren,t,NULL,&d);SDL_DestroyTexture(t);}SDL_FreeSurface(su);}}
            ...
            SDL_Surface *su=TTF_RenderText_Blended(ctx->fntTiny,l1,c1);if(su){SDL_Texture *t=SDL_CreateTextureFromSurface(ren,su);SDL_Rect d={1620,593,su->w,su->h};if(t){SDL_RenderCopy(ren,t,NULL,&d);SDL_DestroyTexture(t);}SDL_FreeSurface(su);}
            su=TTF_RenderText_Blended(ctx->fntTiny,l2,c2);if(su){SDL_Texture *t=SDL_CreateTextureFromSurface(ren,su);SDL_Rect d={1620,615,su->w,su->h};if(t){SDL_RenderCopy(ren,t,NULL,&d);SDL_DestroyTexture(t);}SDL_FreeSurface(su);}}
            SDL_SetRenderDrawColor(ren,CY_R,CY_G,CY_B,28);SDL_RenderDrawLine(ren,1620,643,1878,643);
            {SDL_Color tc={CY_R,CY_G,CY_B,255};SDL_Surface *su=TTF_RenderText_Blended(ctx->fntTiny,"AUDIO:",tc);
            if(su){SDL_Texture *t=SDL_CreateTextureFromSurface(ren,su);SDL_Rect d={1620,652,su->w,su->h};if(t){SDL_RenderCopy(ren,t,NULL,&d);SDL_DestroyTexture(t);}SDL_FreeSurface(su);}}
            ...
            {SDL_Rect bar={1697+b*17,678-bh,12,bh};SDL_RenderFillRect(ren,&bar);}}}
        }
        ...
        {SDL_Rect bot={0,WIN_H-30,WIN_W,30};SDL_RenderFillRect(ren,&bot);}
