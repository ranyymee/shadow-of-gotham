...
{
    int kLeft, kRight, kUp, kFly, kPunch, kKick, kShoot, kCrouch, moving, i;

    if (!p->isAlive) return;

    kLeft = 0; kRight = 0; kUp = 0; kFly = 0;
    kPunch = 0; kKick = 0; kShoot = 0; kCrouch = 0;

    for (i = 0; i < numKeys; i++) {
        if (keys_pressed[i] == cfg.left)   kLeft   = 1;
        if (keys_pressed[i] == cfg.right)  kRight  = 1;
        if (keys_pressed[i] == cfg.up)     kUp     = 1;
        if (keys_pressed[i] == cfg.fly)    kFly    = 1;
    ...
    if (p->onGround && kCrouch) p->isCrouching = 1;
    ...
    if (!p->isCrouching) {
        ...
        if (moving) {
            double spd = kCrouch ? 380.0 : 220.0;
            p->vitesse = p->direction ? spd : -spd;
            if (kCrouch) p->isRunning = 1;
        }
    }
                   ...
                   const char *winner, int scoreP1, int scoreP2)
{
    ...
    while (1) {
        ...
        s = TTF_RenderUTF8_Blended(font, "GAME OVER", red);
        if (s) { t = SDL_CreateTextureFromSurface(ren, s); d.x = SCREEN_W/2 - s->w/2; d.y = 160; d.w = s->w; d.h = s->h; SDL_RenderCopy(ren, t, NULL, &d); SDL_FreeSurface(s); SDL_DestroyTexture(t); }

        snprintf(buf, sizeof(buf), "%s GAGNE !", winner);
        s = TTF_RenderUTF8_Blended(font, buf, yellow);
        if (s) { t = SDL_CreateTextureFromSurface(ren, s); d.x = SCREEN_W/2 - s->w/2; d.y = 230; d.w = s->w; d.h = s->h; SDL_RenderCopy(ren, t, NULL, &d); SDL_FreeSurface(s); SDL_DestroyTexture(t); }

        snprintf(buf, sizeof(buf), "BATMAN   : %d pts", scoreP1);
        s = TTF_RenderUTF8_Blended(font, buf, white);
        if (s) { t = SDL_CreateTextureFromSurface(ren, s); d.x = SCREEN_W/2 - s->w/2; d.y = 300; d.w = s->w; d.h = s->h; SDL_RenderCopy(ren, t, NULL, &d); SDL_FreeSurface(s); SDL_DestroyTexture(t); }

        snprintf(buf, sizeof(buf), "CATWOMAN : %d pts", scoreP2);
        s = TTF_RenderUTF8_Blended(font, buf, white);
        if (s) { t = SDL_CreateTextureFromSurface(ren, s); d.x = SCREEN_W/2 - s->w/2; d.y = 330; d.w = s->w; d.h = s->h; SDL_RenderCopy(ren, t, NULL, &d); SDL_FreeSurface(s); SDL_DestroyTexture(t); }

        s = TTF_RenderUTF8_Blended(font, "[ ENTREE ] - Rejouer", green);
        if (s) { t = SDL_CreateTextureFromSurface(ren, s); d.x = SCREEN_W/2 - s->w/2; d.y = 410; d.w = s->w; d.h = s->h; SDL_RenderCopy(ren, t, NULL, &d); SDL_FreeSurface(s); SDL_DestroyTexture(t); }

        s = TTF_RenderUTF8_Blended(font, "[ ECHAP  ] - Quitter", red);
        if (s) { t = SDL_CreateTextureFromSurface(ren, s); d.x = SCREEN_W/2 - s->w/2; d.y = 450; d.w = s->w; d.h = s->h; SDL_RenderCopy(ren, t, NULL, &d); SDL_FreeSurface(s); SDL_DestroyTexture(t); }
...
int gameLoop(SDL_Renderer *ren, TTF_Font *font, SDL_Texture *bg)
{
    ...
    cfg1.left   = SDL_SCANCODE_LEFT;
    cfg1.right  = SDL_SCANCODE_RIGHT;
    cfg1.up     = SDL_SCANCODE_UP;
    cfg1.fly    = SDL_SCANCODE_SPACE;
    cfg1.punch  = SDL_SCANCODE_KP_1;
    cfg1.kick   = SDL_SCANCODE_KP_2;
    cfg1.shoot  = SDL_SCANCODE_KP_3;
    cfg1.crouch = SDL_SCANCODE_DOWN;

    cfg2.left   = SDL_SCANCODE_A;
    cfg2.right  = SDL_SCANCODE_D;
    cfg2.up     = SDL_SCANCODE_W;
    cfg2.fly    = SDL_SCANCODE_W;
    cfg2.punch  = SDL_SCANCODE_F;
    cfg2.kick   = SDL_SCANCODE_E;
    cfg2.shoot  = SDL_SCANCODE_H;
    cfg2.crouch = SDL_SCANCODE_S;
