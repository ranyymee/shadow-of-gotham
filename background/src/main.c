...
/* Background = 1600x427 — on garde la fenetre a 1000x427
   GROUND_Y = hauteur background - hauteur personnage (150px) - marge (10px) */
#define SCREEN_W  1000
#define SCREEN_H  427
#define GROUND_Y  267

/* ── Dimensions sprite sheets ──────────────────────────
   Batman:   688x1568  =>  frameW=172  frameH=174
   Catwoman: 672x1550  =>  frameW=168  frameH=172
──────────────────────────────────────────────────────── */
...
/* ════════════════════════════════════════════════════════
   drawHUD — barres HP + scores des deux joueurs
════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════
   loadPlayerTex — charge les sprite sheets d'un joueur
════════════════════════════════════════════════════════ */
                           ...
                           SDL_Texture **outR, SDL_Texture **outL)
{
    ...
    *outL = IMG_LoadTexture(r, pl);
    ...
    /* Dimensions selon le personnage
       initSpriteData s'occupe du SDL_BLENDMODE_BLEND */
    fw = isCat ? CAT_FRAME_W : BAT_FRAME_W;
    fh = isCat ? CAT_FRAME_H : BAT_FRAME_H;
...
/* ════════════════════════════════════════════════════════
   gameLoop — boucle principale (lot1 + lot2 integres)
════════════════════════════════════════════════════════ */
                    ...
                    int affMode)
{
    ...
    initPlayer(&p2, 750, GROUND_Y, 1);   /* Catwoman */
    p2.direction = 0;
    ...
    while (running) {
        ...
        /* Construire tableau SDL_Rect pour la collision */
        {
...
int main(int argc, char *argv[])
{
    ...
    /* Charger background + plateformes level 1 */
    initBackgroundAndPlatforms(renderer, &bg, platforms, &taille, level);

    /* Ne pas redimensionner la fenetre selon le background —
       on garde SCREEN_W x SCREEN_H fixe pour que GROUND_Y soit correct */
    (void)bgW; (void)bgH;
    ...
    /* ── Boucle jeu principale ── */
    while (play) {
        /* Gestion changement de niveau / mode depuis l'ecran principal */
