...
void loadPlayerTextures(SDL_Renderer *ren, Player *p)
{
    ...
    /* ── debug: print load status to stderr ── */
...
/*  handleInput
 *
 *  Movement rules (as requested):
 *   - JUMP key (SPACE)     : parabolic forward jump (horizontal displacement in facing dir)
 *                            Double-press in air (cat only / batman fly) → double jump
 *   - UP key               : vertical hop IN PLACE (parabola, no x displacement)
 *   - CROUCH + hold        : player crouches, stays in crouching position
 *   - CROUCH + direction   : crouching walk (slow)
 *   - Attacks / kick / shoot block movement
 */
void handleInput(Player *p, SDL_Scancode *keys_pressed, InputConfig cfg, int numKeys)
{
    ...
    for (i = 0; i < numKeys; i++) {
        ...
        /* Shift = run modifier: either real Shift key */
        if (keys_pressed[i] == SDL_SCANCODE_LSHIFT ||
            keys_pressed[i] == SDL_SCANCODE_RSHIFT)   kShift  = 1;
    }

    /* While performing an action: freeze horizontal movement */
    ...
    if (!p->jumping && !p->hopping) {
        ...
        /* In air: allow drift, running drift is faster */
    ...
    if (kJump) {
        if (!p->jumpPressed) {
            ...
            if (canJump) {
                ...
                p->hopping       = 0;
                p->jumpCut       = 0;
                p->coyoteTimer   = COYOTE_TIME; /* consume coyote */
                p->canDoubleJump = 1;           /* grant double-jump for both characters */
                p->jumpPressed   = 1;
                p->animState     = ANIM_JUMP;
                p->animFrame     = 0;
            } else if (p->canDoubleJump) {
                /* Double-jump: reaches twice the height of a normal jump */
                p->vy            = JUMP_VY * 2.0f;
                p->jumping       = 1;
                p->hopping       = 0;
                p->jumpCut       = 0;
        ...
        /* Button released — apply jump cut for variable height */
    ...
    /* ── UP key: vertical in-place hop (no x displacement) ── */
    if (kUp && p->onGround && !p->jumping && !p->hopping && !p->isCrouching) {
        ...
        p->jumpCut    = 0;
        p->vy         = HOP_VY;
        p->vitesse    = 0.0;   /* NO horizontal displacement */
        p->animState  = ANIM_UP;
        p->animFrame  = 0;
    }
...
handle_attacks:
    ...
    if (kShoot && !p->isShooting && !p->isAttacking && !p->isKicking) {
        int j;
        /* Spawn bullet at hand level using the rendered sprite rect (posScreen).
           Hand is roughly 38% from the top of the rendered sprite. */
...
void renderBullets(SDL_Renderer *r, Player *p, SDL_Texture *batarang, Uint32 ticks)
{
    int i;
    SDL_Rect br;
    /* spin: full rotation every 300 ms */
    ...
    for (i = 0; i < MAX_BULLETS; i++) {
        ...
        if (batarang && !p->isCat) {
            /* Batman: spinning batarang texture, 40x40 px */
             ...
             MenuResult *menu)
{
    ...
    /* Init players from menu selection */
    ...
    /* Sync fly = jump in configs (menu sets fly, game uses jump) */
    InputConfig cfg1 = menu->inputP1;
    InputConfig cfg2 = menu->inputP2;
    cfg1.jump = cfg1.fly;
    cfg2.jump = cfg2.fly;

    /* batarang bullet sprite */
...
int main(void)
{
    ...
    play = 1;
    while (play) {
        /* Show menu — returns 1=play, 0=quit */
