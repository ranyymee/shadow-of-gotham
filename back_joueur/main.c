...
/* ================================================================
 *  COLLISION AVEC OBSTACLES (Corrigée - ne modifie pas les positions monde)
 * ================================================================ */
                                     ...
                                     Background *bg)
{
    ...
    for (i = 0; i < taille; i++) {
        ...
        /* Convertir la position MONDE de l'obstacle en position ÉCRAN */
        ...
        if (rectsOverlap(playerRect, obstacleScreenRect)) {
            ...
            /* COLLISION PHYSIQUE - on ajuste seulement la position du joueur */
            ...
            /* Ajuster la position du joueur (NE PAS toucher à l'obstacle) */
            switch(collisionSide) {
                case 0: /* collision par la gauche */
                    p->x = obstacleScreenRect.x - p->w;
                    p->vitesse = 0;
                    break;
                case 1: /* collision par la droite */
                    p->x = obstacleScreenRect.x + obstacleScreenRect.w;
                    p->vitesse = 0;
                    break;
                case 2: /* collision par le haut (joueur tombe dessus) */
                    p->y = obstacleScreenRect.y - p->h;
                    p->vy = 0;
                    p->onGround = 1;
                    break;
                case 3: /* collision par le bas (joueur saute dans l'obstacle) */
                    p->y = obstacleScreenRect.y + obstacleScreenRect.h;
                    if (p->vy > 0) p->vy = 0;
                    break;
            }
                               ...
                               InputConfig *cfg, float groundEcran)
{
    ...
    if (keys[cfg->punch] && !p->isAttacking && !p->isKicking
                         && !p->isShooting  && p->actionTimer == 0) {
        p->isAttacking = 1;
        p->actionTimer = 42;
        p->vitesse = 0;
        p->animState = ANIM_ATTACK;
        p->animFrame = 0;
    }

    if (keys[cfg->kick] && !p->isAttacking && !p->isKicking
                        && !p->isShooting  && p->actionTimer == 0) {
        p->isKicking  = 1;
        p->actionTimer = 42;
        p->vitesse = 0;
        p->animState = ANIM_KICK;
        p->animFrame = 0;
    }

    if (keys[cfg->shoot] && !p->isAttacking && !p->isKicking
                         && !p->isShooting  && p->actionTimer == 0) {
        int i;
        p->isShooting  = 1;
        p->actionTimer = 24;
        p->vitesse = 0;
        p->animState = ANIM_SHOOT;
        p->animFrame = 0;
        for (i = 0; i < MAX_BULLETS; i++) {
            if (!p->bullets[i].active) {
                p->bullets[i].active = 1;
                p->bullets[i].owner  = p->isCat ? 2 : 1;
                p->bullets[i].y      = (float)(p->y + p->h * 0.30f);
                if (p->direction) {
                    p->bullets[i].x  = (float)(p->x + p->w + 5);
                    p->bullets[i].vx = 600.0f;
                } else {
                    p->bullets[i].x  = (float)(p->x - 5);
                    p->bullets[i].vx = -600.0f;
                }
                break;
            }
