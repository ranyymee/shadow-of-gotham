...
const CharacterDef CHARACTERS[CHAR_COUNT] = {
    {
        "Batman",
        { "assets/batman_right_1.png", "assets/batman_right_2.png", "assets/batman_right_3.png" },
        { "assets/batman_left_1.png",  "assets/batman_left_2.png",  "assets/batman_left_3.png"  },
    ...
    {
        "Catwoman",
        { "assets/catwoman_right_1.png", "assets/catwoman_right_2.png", "assets/catwoman_right_3.png" },
        { "assets/catwoman_left_1.png",  "assets/catwoman_left_2.png",  "assets/catwoman_left_3.png"  },
    ...
    while (1) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN) {
                ...
                SDL_Rect ok = { screenW/2 - 90, screenH/2 + N * 26 + 16, 180, 42 };
                for (i = 0; i < N; i++) {
                    SDL_Rect btn = { screenW/2 - 125, screenH/2 - 200 + i * 52, 250, 40 };
        ...
        drawTextCentered(renderer, font, title, screenW / 2, 50, yellow);
        ...
        for (i = 0; i < N; i++) {
            ...
            SDL_Color bc;
            const char *label;
            drawText(renderer, font, actionNames[i], screenW/2 - 250, screenH/2 - 188 + i * 52, white);
            if (waiting == i) {
                label = ">>> appuie <<<";
        ...
        {
            SDL_Rect ok = { screenW/2 - 90, screenH/2 - 200 + N * 52 + 16, 180, 42 };
                     ...
                     int screenW, int screenH)
{
    ...
    int CARD_H   = 95;
    int CARD_GAP = 8;
    int CARD_Y0  = screenH / 2 - 180;

    int L1 = screenW / 4 - 210;
    int W1 = 420;
    int L2 = screenW * 3 / 4 - 210;
    int W2 = 420;
    ...
    while (1) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN) {
                int mx = e.button.x, my = e.button.y;
                SDL_Rect btnVal = { screenW/2 - 120, screenH - 120, 240, 44 };
                SDL_Rect btnRet = { screenW/2 - 120, screenH - 68, 240, 40 };
        ...
        drawTextCentered(renderer, font, "COSTUME SELECTION", screenW / 2, 46, yellow);
...
int runMenu(SDL_Renderer *renderer, TTF_Font *font, MenuResult *result)
{
    ...
    bg = IMG_LoadTexture(renderer, "assets/back.png");
    if (!bg) bg = IMG_LoadTexture(renderer, "back.png");
    if (!bg) printf("Avertissement: background menu non charge\n");
    ...
    sel = charSelectScreen(renderer, font, bg, result, avP1, avP2, screenW, screenH);
    if (sel == 1) ret = 1;
    else          ret = 0;
